...
# ==============================================================================
  ...
  resource "aws_subnet" "public" { # 1개만 생성
    count             = length(var.public_subnets) # ip 대역 개수대로 반복 생성
    vpc_id            = aws_vpc.this.id # vpc 위치 지정
    cidr_block        = var.public_subnets[count.index] # 순서대로 CIDR 값 꺼내옴

    availability_zone = var.azs[count.index] # 순서대로 AZ 꺼내옴

    map_public_ip_on_launch = true # ec2 생성 시 자동으로 공인 IP 할당
  ...
  resource "aws_subnet" "app" {
    count             = length(var.app_subnets) # App용 IP 대역 개수만큼 반복
